...
from config import LINKEDIN_LI_AT_COOKIE, LINKEDIN_JSESSIONID_COOKIE

logger = logging.getLogger(__name__)

api = None
try:
    if not LINKEDIN_LI_AT_COOKIE or not LINKEDIN_JSESSIONID_COOKIE:
        logger.error("LinkedIn cookies are missing in config.py!")
    else:
        logger.info("Attempting to initialize LinkedIn API...")
        
        # Pass BOTH cookies to the library
        cookie_dict = {
            'li_at': LINKEDIN_LI_AT_COOKIE,
            'JSESSIONID': LINKEDIN_JSESSIONID_COOKIE
        }
