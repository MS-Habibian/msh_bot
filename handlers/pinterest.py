...
async def pin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = await update.message.reply_text("در حال جستجو، لطفا کمی صبر کنید...")

    # دریافت تعداد بیشتری نتیجه (مثلا ۵۰ تا) برای امکان صفحه‌بندی
    pins = await search_pinterest_rss(query, limit=50)
    
    if not pins:
        await message.edit_text("نتیجه‌ای یافت نشد.")
        return

    # ذخیره اطلاعات برای صفحه‌بندی در مراحل بعد
    context.user_data['pin_query'] = query
    context.user_data['pin_page'] = 1
    context.user_data['pin_results'] = pins

    await message.delete()
    # فراخوانی تابع ارسال برای صفحه اول
    await send_pinterest_page(update.message, context, query, page=1)


async def send_pinterest_page(message, context: ContextTypes.DEFAULT_TYPE, query: str, page: int):
    pins = context.user_data.get('pin_results', [])
    limit = 10
    offset = (page - 1) * limit
    page_pins = pins[offset : offset + limit]

    if not page_pins:
        await message.reply_text("عکس بیشتری برای این جستجو وجود ندارد.")
        return

    # ارسال ۱۰ عکس این صفحه با دکمه دانلود
    ...
    if offset + limit < len(pins):
        keyboard = [[InlineKeyboardButton("⬇️ دریافت 10 تصویر بعدی", callback_data="pin_page|next")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await message.reply_text(f"📌 نتایج جستجوی *{query}* (صفحه {page}):", reply_markup=reply_markup, parse_mode="Markdown")
...
async def pin_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # حذف دکمه "۱۰ تصویر بعدی" از پیام قبلی تا کاربر دوباره روی آن کلیک نکند
    try:
    ...
    search_query = context.user_data.get('pin_query', 'جستجو')

    # ارسال ۱۰ عکس بعدی
    await send_pinterest_page(query.message, context, search_query, page=next_page)
